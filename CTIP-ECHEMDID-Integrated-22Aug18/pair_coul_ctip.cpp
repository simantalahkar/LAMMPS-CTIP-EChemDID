...
PairCoulCtip::~PairCoulCtip()
{
  ...
  if (allocated) {
    ...
    memory->destroy(qeq_qmin);
    memory->destroy(qeq_qmax);
    delete [] map;
  }
}
...
void PairCoulCtip::allocate()
{
 ...
 memory->create(qeq_qmin,n+1,"pair:qeq_qmin");
 memory->create(qeq_qmax,n+1,"pair:qeq_qmax");
...
void PairCoulCtip::settings(int narg, char **arg)
{
  ...
  ctip_omega = force->numeric(FLERR,arg[1]);

  if (strcmp(arg[2],"wolf") == 0){
    kspacetype = 1;
    g_wolf = force->numeric(FLERR,arg[3]);
  } else if (strcmp(arg[2],"ewald") == 0){
...
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style coul/ctip requires atom attribute q");
...
void PairCoulCtip::read_file(char *file)
{
  ...
  if (comm->me == 0) {
    ...
    if (fp == NULL) {
      char str[128];
      sprintf(str,"Cannot open coul/ctip potential file %s",file);
      error->one(FLERR,str);
    }
  }
  ...
  while (1) {
    ...
    if (nwords != params_per_line)
      error->all(FLERR,"Incorrect format in coul/ctip potential file");
    ...
    params[nparams].qmin = atof(words[6]);
    params[nparams].qmax = atof(words[7]);

    // parameter sanity check

    if (params[nparams].eta < 0.0 || params[nparams].zeta < 0.0 ||
        params[nparams].zcore < 0.0 || params[nparams].gamma != 0.0 )
      error->all(FLERR,"Illegal coul/ctip parameter");
...
{
 // qi*X_i + 0.5*qi^2*J_i  (electronegativity + self-Coulomb repulsion)
 // + w(1-(qi-qmin)/|qi-qmin|)(qi-qmin)^2  correction for cation
 // + w(1-(qmax-qi)/|qi-qmax|)(qi-qmax)^2  correction for anion
 // correction terms keep charges in bound
 double qqrd2e = force->qqrd2e;
 double chi=param->chi, eta=param->eta;
 double qmin=param->qmin, qmax=param->qmax;
 double correction;

 if (qi < qmin) 
   correction = 2.0*ctip_omega * (qi-qmin)*(qi-qmin);
 else if (qi > qmax) 
   correction = 2.0*ctip_omega * (qi-qmax)*(qi-qmax);
 else 
   correction = 0.0;

 if (kspacetype == 1) 
   return qi*(chi + qi*(0.50*eta - qqrd2e*woself))
          + correction;

 if (kspacetype == 2) 
   return qi*(chi + qi*0.50*eta)
          + correction;
...
void PairCoulCtip::coulomb_integral_wolf(double zei, double zej, double r,
...
void PairCoulCtip::wolf_sum(double qi, double qj, double zj, double r,
		double ci_jfi, double dci_jfi, double ci_fifj,
...
void PairCoulCtip::coulomb_integral_ewald(double zei, double zej, double r,
		  double &ci_jfi, double &dci_jfi, double &ci_fifj,
		  double &dci_fifj)
{
  double rinv = 1.0/r;
...
void PairCoulCtip::ewald_sum(double qi, double qj, double zj, double r,
		double ci_jfi, double dci_jfi, double ci_fifj,
...
double PairCoulCtip::memory_usage()
{
...
void *PairCoulCtip::extract(const char *str, int &dim)
{
  ...
  if (strcmp(str,"qmin") == 0 && qeq_qmin) {
    dim = 1;
    for (int i = 1; i <= atom->ntypes; i++)
      if (map[i] >= 0) qeq_qmin[i] = params[elem2param[map[i]]].qmin;
      else qeq_qmin[i] = 0.0;
    return (void *) qeq_qmin;
  }
  if (strcmp(str,"qmax") == 0 && qeq_qmax) {
    dim = 1;
    for (int i = 1; i <= atom->ntypes; i++)
      if (map[i] >= 0) qeq_qmax[i] = params[elem2param[map[i]]].qmax;
      else qeq_qmax[i] = 0.0;
    return (void *) qeq_qmax;
  }
  ...
  if (strcmp(str,"omega") == 0) {
    dim = 0;
    return (void *) &ctip_omega;
  }
  if (strcmp(str,"wolfewald") == 0) {
    dim = 0;
